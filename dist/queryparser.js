        ...
        execute: function() {
            ScopedPrometheusQueryParser = (function () {
                function ScopedPrometheusQueryParser() {
                    ...
                    this.parenre = /(\{[^}]+\}(?:\[\d+[hmsdw]\])?)/gi;
                    this.pConst = /^\s*(?:(?:-?(?:(?:\d*\.\d+)|\d+)%?)|true|false|null|__##[0-9a-z]{6}##__)\s*$/i;
                    this.pString = /"[^"]*"/g;
                    this.pq = {};
                    this.txttbl = {};
                    this.re = [
                    ...
                    this.injectScopeIntoQuery = function (query, scope) {
                        console.time("injectScope Timer");
                        var q = query.replace(_this.pString, "\"__$$#$$TRING$$#$$__\"");
                        console.debug("cached query count: %d;; using q= %s", Object.keys(_this.pq).length, q);
                        //console.log(i + ">> Inside injectScopeIntoQuery. with query=%s;;", query );
                        if (!_this.pq.hasOwnProperty(q)) {
                            _this.pq[q] = _this.parseQuery(q, "0");
                            console.debug("Adding to query cache [%s;;] => [%s;;]", q, _this.pq[q]);
                        }
                        var o = new Optimizer(query, _this.pString);
                        var ret = _this.pq[q].replace(_this.pString, o.swap.bind(o)).replace(/\$SCOPE/, scope);
                ...
                ;
                ScopedPrometheusQueryParser.prototype.parseSegment = function (a, b) {
                ...
                ;
                ScopedPrometheusQueryParser.prototype.processStuff = function (s, b) {
                    //console.log(b + ">> Inside processStuff. with s=" + s + ";;");
                    b = b + "-";
                    //split by operator
                    var ab = s;
                    if (this.pCOps.test(ab) || this.pMOps.test(ab) || this.pBOps.test(ab))
                        ab = this.processOps(ab);
