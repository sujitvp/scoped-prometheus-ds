...
import ScopedPrometheusMetricFindQuery from '../metric_find_query';

describe('ScopedPrometheusMetricFindQuery', function() {
  ...
  beforeEach(
    angularMocks.inject(function($q, $rootScope, $httpBackend, $injector) {
      ...
      ctx.ds = $injector.instantiate(ScopedPrometheusDatasource, {
  ...
  describe('When performing metricFindQuery', function() {
    ...
    it('label_values(resource) should generate label search query', function() {
      ...
      var pm = new ScopedPrometheusMetricFindQuery(ctx.ds, 'label_values(resource)', ctx.timeSrv);
    ...
    it('label_values(metric, resource) should generate series query', function() {
      ...
      var pm = new ScopedPrometheusMetricFindQuery(ctx.ds, 'label_values(metric, resource)', ctx.timeSrv);
      pm.process().then(function(data) {
        results = data;
      });
      ctx.$httpBackend.flush();
    ...
    it('metrics(metric.*) should generate metric name query', function() {
      ...
      var pm = new ScopedPrometheusMetricFindQuery(ctx.ds, 'metrics(metric.*)', ctx.timeSrv);
      pm.process().then(function(data) {
        results = data;
      });
      ctx.$httpBackend.flush();
    ...
    it('query_result(metric) should generate metric name query', function() {
      ...
      var pm = new ScopedPrometheusMetricFindQuery(ctx.ds, 'query_result(metric)', ctx.timeSrv);
      pm.process().then(function(data) {
        results = data;
      });
      ctx.$httpBackend.flush();
